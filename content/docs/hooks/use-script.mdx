...
| `options.customAttributes` | `Record<string, string>` | `customAttributes` which is an object of custom string attributes that will be added to the script tag, this can be useful for specifying things like license keys in `data-x` attributes. |
