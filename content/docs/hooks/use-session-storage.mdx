...
```bash
npx shadcn@latest add https://usekit.kiron.dev/r/use-session-storage
```
...
```tsx
const [authToken, setAuthToken] = useSessionStorage<string>(
  "auth-token",
...
| -------------- | --------- | --------------------------------------------------------------------------- |
| `key`          | `string`  | The key used to access the sessionStorage value.                            |
| `initialValue` | `T`       | The initial value to use if there is no item in the sessionStorage with the provided key. |
