...
export function Component() {
  ...
  const { error, data, loading } = useFetch(`https://pokeapi.co/api/v2/pokemon/${count}`)
...
| `options?` | `object` | Additional options for the fetch request, such as headers or request methods. |
