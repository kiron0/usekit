...
```bash
npx shadcn@latest add https://usekit.kiron.dev/r/use-history-state
```
...
| ---------------- | ---- | ----------------------------------------------------------------------------------------- |
| `initialPresent` | `T`  | (Optional) The initial state for the `present` value. Defaults to an empty object (`{}`). |
